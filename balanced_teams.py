...
def sort_dataframe(df):
    sorted_df = df.sort_values(by=[AGE, SEX, WEEK1, WEEK2, WEEK3], ascending=False)
    sorted_df.to_csv('sorted_participants.csv', index=False)
    return sorted_df
...
def print_team_stats(teams, participants):
...
def main():
    ...
    teams = distribute_participants(participants, n_teams=4)

    print_teams(teams)

    print_team_stats(teams, participants)
